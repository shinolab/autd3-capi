...
use autd3capi_driver::{
    autd3::link::audit::{Audit, version::Latest},
    *,
};
...
pub unsafe extern "C" fn AUTDLinkAudit() -> LinkPtr {
    Audit::latest(autd3::link::AuditOption::default()).into()
}
...
pub unsafe extern "C" fn AUTDLinkAuditFpgaIsForceFan(audit: LinkPtr, idx: u16) -> bool {
    audit.cast::<Audit<Latest>>()[idx as usize]
        .fpga()
        .is_force_fan()
}
...
pub unsafe extern "C" fn AUTDLinkAuditCpuSilencerStrictMode(audit: LinkPtr, idx: u16) -> bool {
    audit.cast::<Audit<Latest>>()[idx as usize].silencer_strict_mode()
}
...
pub unsafe extern "C" fn AUTDLinkAuditFpgaGPIOOutputTypes(audit: LinkPtr, idx: u16, ty: *mut u8) {
    unsafe {
        let src = audit.cast::<Audit<Latest>>()[idx as usize]
            .fpga()
            .gpio_out_types();
...
pub unsafe extern "C" fn AUTDLinkAuditFpgaPulseWidthEncoderTable(
    ...
    unsafe {
        let dst = std::slice::from_raw_parts_mut(
            ...
            driver::firmware::latest::fpga::PWE_BUF_SIZE,
        );
        let fpga = audit.cast::<Audit<Latest>>()[idx as usize].fpga();
